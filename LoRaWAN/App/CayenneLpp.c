...
uint8_t CayenneLppDeviceIdToBufferSize(uint8_t deviceId){
	if(deviceId == 0)
		return 3;
	else if(deviceId == 1)
		return 3;
		...
		return 4;
	else if(deviceId == 103)
		return 4;
	else if(deviceId == 113)
		return 8;

	return 0;
}

void CayenneLppDecode(uint8_t BufferSize, uint8_t *Buffer){

	int i = 0;

	uint8_t deviceId = Buffer[i];
	uint8_t channel = Buffer[++i];
	uint8_t bufferSize = CayenneLppDeviceIdToBufferSize(deviceId);
	uint8_t dataBufferSize = bufferSize - 2;
	uint8_t *dataBuffer = malloc(dataBufferSize * sizeof(uint8_t));
	for(int j = 0; j < dataBufferSize; j++){
		dataBuffer[j] = Buffer[i+j+2];
		i++;
	}
}
